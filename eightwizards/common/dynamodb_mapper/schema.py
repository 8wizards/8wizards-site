...
from .types import ALLOWED_DYNAMODB_TYPES, TypeSerializer
from .errors import Invalid, InvalidList, UNDEFINED
...
class Schema(object):
    ...
    def validate(self, path, schema, data):
        ...
        try:
            ...
            return self._validate_dynamodb_type(path, schema, data)
        except InvalidList:
            raise
        except Invalid as e:
            raise InvalidList([e])
    ...
    def _validate_dict(self, path, schema, data):
        ...
        from .validators import Required, Optional, extra_validator
        ...
        for skey in schema:
            if isinstance(skey, Required) or self.required and not isinstance(skey, Optional):
        ...
        for key, value in data.items():
            ...
            for skey, rule in schema.items():
                if skey is extra_validator:
    ...
    @staticmethod
    def _validate_dynamodb_type(path, schema, data):
        """Validate a scalar value of data comparing to the dynamodb type.
        Can be: value, dynamodb type
        """
        # value specification ?
        if data == schema:
            pass

        elif schema in ALLOWED_DYNAMODB_TYPES:
            if isinstance(data, datetime):
                data = data.replace(tzinfo=utc_tz).isoformat()
            if isinstance(data, date):
                data = data.isoformat()

            serializer = TypeSerializer()
            try:
                type = serializer._get_dynamodb_type(value=data)
                if type != schema:
                    raise Invalid('%s is not a valid scalar or dynamodb type value' % (repr(data)), path)
            except TypeError:
                raise Invalid('%s is not a valid scalar or dynamodb type value' % (repr(data)), path)
        else:
            raise Invalid('%s is not a valid scalar or dynamodb type value' % (repr(data)), path)
    ...
    def _validate_scalar(path, schema, data):
        ...
        # type specification ?
            ...
            if not isinstance(data, schema):
                raise Invalid('expected value of type %s. Got %s of type %s' % (schema.__name__, repr(data),
                                                                                type(data)), path)
